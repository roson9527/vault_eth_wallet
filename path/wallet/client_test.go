...
func TestClient_CreateAndDelete(t *testing.T) {
	...
	fmt.Println(sec, err)
	sec, err = cli.Logical().Delete("mock_web3/" + fmt.Sprintf(PatternWallet, NameSpaceGlobal, address))
	if err != nil {
		panic(err)
	}
	fmt.Println(sec, err)
...
func TestClient_Update(t *testing.T) {
	...
	fmt.Println(sec, err)
	sec, err = cli.Logical().Write("mock_web3/"+fmt.Sprintf(PatternWallet, NameSpaceGlobal, address), map[string]any{
...
func TestClient_List(t *testing.T) {
	cli := createLocalClient()
	sec, err := cli.Logical().List("mock_web3/" + fmt.Sprintf(PatternWallet, "test_b", ""))
	if err != nil {
		panic(err)
	}
	fmt.Println(sec, err)
